...
from numpy import arcsin, arctan, sin, cos, pi   # Angle in radians for using numpy sin and cos calculation
import pandas as pd
import matplotlib.pyplot as plt
...
def output_mechprops(strap_length, theta_0, n_elements):
    ...
    theta_0_rad = math.radians(theta_0) # convert from degrees to radians, starting angle of the wave
    #calculated parameters
    wave_length = period_length/2           # wave length in mm
    wave_height = (wave_length/2)*math.tan(theta_0_rad)        # wave height in mm, theta_o_rad in radians
...
def MeshMechanics (osimModel, theta_0_values, n_elements):
    ...
    for i, theta_0 in enumerate(theta_0_values):
        ...
        slippage_strain=slippage / strap_length                  # Convert the slippage displacement to strain
        FL_matrix[0] = FL_matrix[0] + slippage_strain    # Add skin movements and slippage strain to the horizontal axis of Force-Length curve
        FL_firstelement=[[1, 1+slippage_strain/2], [0,0]]                              # The first element added to the Force-Length curve due to skin movements and slippage
        ...
        FL_fatigue=[[max(FL_matrix[0]), 1.5], [0, 0]]             # After the fatigue length, the force will turn to 0
        FL_matrix=[list(FL_matrix[0])+FL_fatigue[0], list(FL_matrix[1])+FL_fatigue[1]]  # Add FL_fatigue to the end of the FL curve
        FL_matrix=[np.array(FL_matrix[0]), np.array(FL_matrix[1])]         # Transfer the Force-Length curve matrix from list to array
        """
