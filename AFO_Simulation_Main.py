...
# Design parameters of force-length amplification
for af_am_1, af_am_2, af_am_3, af_am_4 in itertools.product(range(2, 4), range(8, 9), range(1, 2), range(3, 4)):
    # Design parameters of force-length shift
    for af_shift_1, af_shift_2, af_shift_3, af_shift_4 in itertools.product(range(1, 2), range(0, 1), range(2, 3), range(2, 3)):
        for strip_ori_1, strip_ori_2, strip_ori_3, strip_ori_4 in itertools.product(range(0, 1), range(0, 1), range(0, 1), range(0, 1)):
            ...
            simulationresults_folder = 'SimulationOutput_DL_'+str(af_am_1)+str(af_am_2)+str(
                af_am_3)+str(af_am_4)+str(af_shift_1)+str(af_shift_2)+str(af_shift_3)+str(af_shift_4)
            output_folder = os.path.join(SelectionPath, simulationresults_folder)
            # put the specified results into a matrix
            data = AFO4_ResultsCollection.Simulationresultscollection(
                output_folder, Results_parameter, 'default_states_degrees.mot')
            # If the folder does not exit, skip the current loop to the next one.
            if data.size == 0:
                continue
            else:
                # transform the variables to numbers
                Var_to_num = af_am_1*pow(10, 7)+af_am_2*pow(10, 6)+af_am_3*pow(10, 5)+af_am_4*pow(
                    10, 4)+af_shift_1*pow(10, 3)+af_shift_2*pow(10, 2)+af_shift_3*pow(10, 1)+af_shift_4
                Subtalar_matrix_DL.append([Var_to_num, af_am_1, af_am_2, af_am_3, af_am_4, af_shift_1*0.2-0.2, af_shift_2*0.2-0.2, af_shift_3 *
                                          0.2-0.2, af_shift_4*0.2-0.2, max(data[:, 1]), max(data[:, 2])])       # Put the four variables and the subtalar angles into a list
# Transform the list to the matrix
Subtalar_matrix_DL = np.array(Subtalar_matrix_DL)
Excel_title = ['Variables in number', 'am_front_lateral', 'am_side_lateral', 'am_side_medial', 'am_front_lateral', 'shift_front_lateral',
               'shift_side_lateral', 'shift_side_medial', 'shift_front_lateral', 'Max subtalar angle', 'Max ankle angle']              # Define the title of the excel
# Put the four variables and subtalar angles to an excel
AFO4_ResultsCollection.DLResultstoExcel(
    SelectionPath, 'Results.xls', 'DL_Platform 25', Excel_title, Subtalar_matrix_DL)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
"""
...
# The specified parameter to extract
Results_parameter = ['time', '/jointset/subtalar_r/subtalar_angle_r/value',
...
root.withdraw()
SelectionPath = filedialog.askdirectory()                       # the path of the selected directory
Activity_folder = SelectionPath+'\SimulationOutput_Walk_'

# The folder path for model without AFO
output_folder_data_withoutAFO = Activity_folder+'00000000'
data_withoutAFO = AFO4_ResultsCollection.Simulationresultscollection(
    output_folder_data_withoutAFO, Results_parameter, Activity_results_file)
...
for af_am_1, af_am_2, af_am_3, af_am_4 in itertools.product(range(2, 4), range(8, 9), range(1, 2), range(3, 4)):
    ...
    for af_shift_1, af_shift_2, af_shift_3, af_shift_4 in itertools.product(range(1, 2), range(0, 1), range(2, 3), range(2, 3)):
        for strip_ori_1, strip_ori_2, strip_ori_3, strip_ori_4 in itertools.product(range(0, 1), range(0, 1), range(0, 1), range(0, 1)):
            output_folder_data_AFO = Activity_folder+str(af_am_1)+str(af_am_2)+str(af_am_3)+str(
                af_am_4)+str(af_shift_1)+str(af_shift_2)+str(af_shift_3)+str(af_shift_4)
            data_AFO = AFO4_ResultsCollection.Simulationresultscollection(
                output_folder_data_AFO, Results_parameter, Activity_results_file)
            # If the folder does not exit, skip the current loop to the next one.
            if data_AFO.size == 0:
                continue
            else:
                ...
                [diff_total_subtalar, diff_average_subtalar, diff_max_subtalar] = AFO4_ResultsCollection.curvecomparison(
                    data_withoutAFO, data_AFO, 1, len(data_withoutAFO))
                ...
                [diff_total_ankle, diff_average_ankle, diff_max_ankle] = AFO4_ResultsCollection.curvecomparison(
                    data_withoutAFO, data_AFO, 2, len(data_withoutAFO))
                # transform the variables to numbers
                Var_to_num = af_am_1*pow(10, 7)+af_am_2*pow(10, 6)+af_am_3*pow(10, 5)+af_am_4*pow(
                    10, 4)+af_shift_1*pow(10, 3)+af_shift_2*pow(10, 2)+af_shift_3*pow(10, 1)+af_shift_4
                Subtalar_matrix_activity.append([Var_to_num, af_am_1, af_am_2, af_am_3, af_am_4, af_shift_1*0.2-0.2, af_shift_2*0.2-0.2, af_shift_3*0.2 -
                                                0.2, af_shift_4*0.2-0.2, diff_average_subtalar, diff_average_ankle])       # Put the four variables and the subtalar angles into a list

# Transform the list to the matrix
Subtalar_matrix_activity = np.array(Subtalar_matrix_activity)
Excel_title = ['Variables in number', 'am_front_lateral', 'am_side_lateral', 'am_side_medial', 'am_front_lateral', 'shift_front_lateral', 'shift_side_lateral',
               'shift_side_medial', 'shift_front_lateral', 'Subtalar angle differences', 'Ankle angle differences']              # Define the title of the excel
# Put the four variables and subtalar angles to an excel
AFO4_ResultsCollection.DLResultstoExcel(SelectionPath, 'Results.xls', Activity, Excel_title, Subtalar_matrix_activity)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
"""
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
AFO4_ResultsCollection.Excel3Dplot(sheet_name='Gait simulation', xs_variable='am_front_lateral',
                                   ys_variable='am_side_lateral', zs_variable='Ankle angle differences')
